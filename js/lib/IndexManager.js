...
*
* An implicit 'byId' index, which can be retrieved via `indexManager.getIndex('byId')` (or whatever you configure as
...
const IndexManager = class {
  ...
  * - className: (optional) the name item class; e.g. 'car', 'animal', etc. Used to produce error messages and defaults *     to the class name set when creating IndexManager. Generally, it's recommended to set the class name when
  *     creating the IndexManager instance rather than here, though there may be some cases where it is useful to
